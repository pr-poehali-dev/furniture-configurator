...
Верни СТРОГО валидный JSON без markdown, без пояснений, в формате:
{
  ...
  "categories": ["tables" | "sofas" | "wardrobes" | "kitchens" | "beds" | "storage"],
  "comment": "тёплый совет дизайнера на русском: что хорошо впишется и почему (2-3 предложения)"
}

Где categories: tables=столы, sofas=диваны/кресла, wardrobes=шкафы/комоды, kitchens=кухни,
beds=кровати, storage=стеллажи. Выбери 1-3 наиболее подходящие категории под этот тип комнаты.
